...
@section('title', 'Postingan Portofolio')
        ...
        <div class="row">
            <div class="col-12 mt-4">
                <div class="d-grid gap-2 d-md-flex">
                    <a href="{{ route('portfolio-posts.create') }}" class="btn btn-primary mb-3 btn-sm">
                        Tambah Data +
                    </a>
                </div>
                            ...
                            <tbody>
                                ...
                                @foreach($posts as $post)
                                    <tr>
                                        ...
                                        <td class="p-3"><img src="{{ asset('storage/portfolio/' . $post->image) }}" width="70px" class="img-fluid" alt="image"></td>
                                        <td class="p-3">{{ $post->title }}</td>
                                        <td class="p-3">{{ $post->Category->title }}</td>
                                        <td style="width: 25%;">
                                            <a href="{{ route('portfolio-posts.edit', $post->id) }}" class="btn btn-warning btn-sm mb-2"><i class="fa-solid fa-pen"></i> Edit</a>
                                            <button type="button" class="btn btn-danger btn-sm mb-2 btn-delete" data-id="{{ $post->id }}"><i class="fa-solid fa-trash"></i> Hapus</button>
                                        </td>
                                    </tr>
                                @endforeach
                                <!-- End -->
